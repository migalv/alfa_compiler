...
typedef enum { ENTERO, BOOLEANO, ERROR } TIPO;

/* Clase de un símbolo: pueden ser variables atómicas (escalares) o listas/arrays (vectores) */
typedef enum { ESCALAR, VECTOR } CLASE;

/* Ambito de la variable que se busca, es util para la busqueda de ambito */
typedef enum {GLOBAL, LOCAL, ERRAMB} AMBITO;
...
INFO_SIMBOLO *crear_info_simbolo(const char *lexema, CATEGORIA categ, TIPO tipo, CLASE clase, int id, int adic1, int adic2);
...
STATUS insertar_simbolo(TABLA_HASH *th, const char *lexema, CATEGORIA categ, TIPO tipo, CLASE clase, int id, int adic1, int adic2);
